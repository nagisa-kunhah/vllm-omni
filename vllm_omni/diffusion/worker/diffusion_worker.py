...
def _clear_model_runner_graphs(model_runner: object | None, *, remove: bool) -> bool:
    """Clear optional graph state exposed by a model runner."""
    clear_graph_runners = getattr(model_runner, "clear_graph_runners", None)
    if callable(clear_graph_runners):
        clear_graph_runners(remove=remove)
        return True
    return False
...
class DiffusionWorker:
    ...
    def sleep(self, level: int = 1) -> int:
        ...
        if level == 2 and self.model_runner is not None:
            if _clear_model_runner_graphs(self.model_runner, remove=False):
    ...
    def shutdown(self) -> None:
        ...
        try:
            if self.model_runner is not None:
                _clear_model_runner_graphs(self.model_runner, remove=True)
