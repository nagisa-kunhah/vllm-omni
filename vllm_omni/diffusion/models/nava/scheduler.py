...
class FlowUniPCMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    # Modified from
    # diffusers.schedulers.scheduling_flow_match_euler_discrete.FlowMatchEulerDiscreteScheduler.set_timesteps
    def set_timesteps(
        self,
        ...
        **kwargs,
        ...
        if prev_timestep is not None:
            deprecate(
                "prev_timestep",
                "1.0.0",
                "Passing `prev_timestep` is deprecated and has no effect as model output conversion "
                "is now handled via an internal counter `self.step_index`",
            )
        ...
        **kwargs,
        ...
        if this_timestep is not None:
            deprecate(
                "this_timestep",
                "1.0.0",
                "Passing `this_timestep` is deprecated and has no effect as model output conversion "
                "is now handled via an internal counter `self.step_index`",
            )
...
class NAVAFlowMatchScheduler(FlowUniPCMultistepScheduler):
    ...
    The numerical implementation is copied from upstream NAVA's
    ``FlowUniPCMultistepScheduler``. The wrapper keeps vLLM-Omni call sites
    simple: ``set_timesteps`` returns the active timesteps and ``step`` returns
    the previous sample tensor directly.
    """
    ...
    def set_timesteps(self, num_inference_steps: int, *, device: torch.device | str | None = None):
